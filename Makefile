...
DESTPATH=$(PREFIX)/share/shalarm

install:
	install -g 0 -o 0 -m 0755 $(SRC)/$(SRCFILE) $(PREFIX)/bin/$(DESTFILE)
	install -v -D -g 0 -o 0 -m 0644 $(DATA)/ring.wav $(DESTPATH)/ring.wav
	install -v -g 0 -o 0 -m 0644 $(DATA)/shalarm.cfg $(DESTPATH)/shalarm.cfg
	install -g 0 -o 0 -m 0644 $(DOC)/shalarm.1 $(MANPATH)

uninstall:
	rm -f $(PREFIX)/bin/$(DESTFILE)
	rm -f $(DESTPATH)/ring.wav
	rm -f $(DESTPATH)/shalarm.cfg
